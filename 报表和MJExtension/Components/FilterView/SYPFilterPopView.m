...
- (SYPTabController *)tabContorller {
    if (!_tabContorller) {
        _tabContorller = [[SYPTabController alloc] initWithFrame:CGRectMake(0, 48, SYPViewWidth, 32)];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        bgView = [[UIView alloc] initWithFrame:CGRectMake(0, SYPViewHeight/2, SYPViewWidth, SYPViewHeight/2)];
        bgView.backgroundColor = SYPColor_BackgroudColor_White;
        bgView.tag = -1001;
        [self addSubview:bgView];
        
        UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 200, 48)];
        ...
        UIView *btmLine = [[UIView alloc] initWithFrame:CGRectMake(0, self.tabContorller.y + self.tabContorller.height - 1, SYPViewWidth, 1)];
...
- (void)setFilter:(SYPFilterModel *)filter {
    if (![filter isEqual:_filter]) {
        ...
        // 选项页显示，并绑定数据
        SYPFilterListView *listView = [[SYPFilterListView alloc] initWithFrame:CGRectMake(0, self.tabContorller.y + self.tabContorller.height, self.width, bgView.height - (self.tabContorller.y + self.tabContorller.height))];
