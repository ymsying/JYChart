...
@implementation SYPMainViewCell
...
- (instancetype)initWithStyle:(SYPMainViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        [self addLine];
        switch (style) {
            case SYPMainViewCellStyleDefault:
            {
                _style = style;
                ...
                break;
            case SYPMainViewCellStyleCustom:
            {
                _style = style;
            }
                break;
...
- (void)layoutSubviews {
    ...
    if (self.style == SYPMainViewCellStyleDefault) {
        [self.titleLabel setFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    }
    if (self.separatorStyle == SYPMainViewCellSeparatorStyleSingleLine) {
        [self setLine];
    }
}
