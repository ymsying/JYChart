...
#import "SYPSeriesModel.h"

@interface SYPPortraitBar () {
...
@property (nonatomic, strong) SYPSeriesModel *seriesModel;

@end

@implementation SYPPortraitBar
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        maxValue = 0.0;
        barWidth = kBarHeight;
        _normalColor = SYPColor_ArrowColor_Green;
        _selectColor = @[SYPColor_ArrowColor_Red, SYPColor_LineColor_LightBlue];
        _dataSource = [NSArray array];
    }
    return self;
}

- (SYPSeriesModel *)seriesModel {
    if (!_seriesModel) {
        _seriesModel = (SYPSeriesModel *)self.model;
    }
    return _seriesModel;
}
...
- (void)drawBarViewWithPoints:(NSArray <NSArray *> *)points {
    ...
    for (NSInteger i = 0; i < points.count; i++) {
        ...
        for (int j = 0; j < self.pointList[i].count; j++) {
            ...
            [path moveToPoint:CGPointMake(CGPointFromString(points[i][j]).x, SYPViewHeight)];
...
- (void)formatDataSource {
    ...
    if (self.seriesModel.maxLength >= 5) {
        barWidth = SYPViewWidth / (self.seriesModel.maxLength * 2 * 1.5);
    }
    ...
    for (NSInteger i = 0; i < self.dataSource.count; i++) {
        ...
        for (int j = 0; j < self.dataSource[i].count; j++) {
            ...
            CGFloat barX = SYPDefaultMargin + (barWidth + barWidth + barWidth) * j + i * (barWidth + 0.5);
            CGFloat barY = 0.0;
            barY = SYPViewHeight - (SYPViewHeight * (value / maxValue));
    ...
    if (self.dataSource.count >= 5) {
        frame.size.width = CGPointFromString([[self.pointList firstObject] lastObject]).x + barWidth / 2 + SYPDefaultMargin;
        self.frame = frame;
    }
