...
@interface SYPSortItemView()

@property (nonatomic, weak) SYPSortButton *selectButton;
@property (nonatomic, weak) SYPSortButton *otherButton;
...
@implementation SYPSortItemView
...
- (void)setupPositionViewsAndItemsWithKeys:(NSArray *)keys{
    ...
    for (NSString *title in keys) {
        SYPSortButton *item = [[SYPSortButton alloc]init];
        item.tag = num;
...
- (void)setSelectButtonTitle:(NSString *)selectButtonTitle{
    ...
    [self.itemsDic enumerateKeysAndObjectsUsingBlock:^(NSString *key, SYPSortButton *button, BOOL *stop) {
...
- (SYPSortButton *)getSortButtonWithTitle:(NSString *)title{
    return [self.itemsDic objectForKey:title];
}

- (void)layoutItemsAfterDeletItem:(SYPSortButton *)item{
    ...
    for (int i = index; i < self.tmpKeys.count-1; i++) {
        ...
        SYPSortButton *nextButton = [self getSortButtonWithTitle:self.tmpKeys[i + 1]];
    ...
    [[SYPItemManager shareitemManager] setItemTitles:self.tmpKeys];
...
- (void)longPress:(UILongPressGestureRecognizer *)longGesture{
    ...
    if (longGesture.state == UIGestureRecognizerStateBegan) {
        ...
        [[SYPItemManager shareitemManager] setItemTitles:self.tmpKeys];
        [[SYPItemManager shareitemManager] printTitles];
...
- (void)animationBetweenSelectItemIndex:(NSInteger)selectIndex AndOtherItemIndex:(NSInteger)otherIndex{
    ...
    if (selectIndex < otherIndex) {
        for (int i = (int)selectIndex + 1; i <= otherIndex; i++) {
            SYPSortButton *item = [self.itemsDic objectForKey:self.tmpKeys[i]];
            [needMoveItem addObject:item];
            UIView *view = self.positionViews[i - 1];
            [positionView addObject:view];
        }
        int j = 0;
        for (SYPSortButton *item in needMoveItem) {
        ...
        for (int i = j; i >= 0; i--) {
            ...
            SYPSortButton *item = needMoveItem[i];
            [UIView animateWithDuration:0.3 animations:^{
                item.frame = view.frame;
            }completion:^(BOOL finished) {
                if (i == 0) {
...
- (void)itemsScare{
    self.isScareing = YES;
    [self.itemsDic enumerateKeysAndObjectsUsingBlock:^(NSString *key, SYPSortButton *item, BOOL *stop) {
...
- (void)itemClick:(SYPSortButton *)item{
    if (item.isScareing) {
...
- (SYPSortButton *)getSortButtonWithKeyIndex:(NSInteger)index{
...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < col ; i++) {
        ...
        for (int j = 0; j < RowNum ; j++) {
            ...
            SYPSortButton *button = [self getSortButtonWithKeyIndex:count];
