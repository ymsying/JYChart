...
#define SYPRootScrollViewDefaultMargin 0

@interface SYPRootScrollView()
...
@property (nonatomic, strong) SYPRootScrollViewManager *manager;
@end

@implementation SYPRootScrollView
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.manager = [[SYPRootScrollViewManager alloc]initWithRootScrollView:self];
...
- (void)reloadPageViews{
    ...
    CGFloat topMargin = [self marginForType:SYPRootScrollViewMarginTypeTop];
    CGFloat bottomMargin = [self marginForType:SYPRootScrollViewMarginTypeBottom];
    CGFloat leftMargin = [self marginForType:SYPRootScrollViewMarginTypeLeft];
    CGFloat rightMargin = [self marginForType:SYPRootScrollViewMarginTypeRight];
...
- (id)dequeueReusableCellWithIdentifier:(NSString *)identifier{
    __block SYPRootScrollViewCell *reusableCell = nil;
    [self.reusePageViews enumerateObjectsUsingBlock:^(SYPRootScrollViewCell *cell, BOOL *stop) {
...
- (CGFloat)marginForType:(SYPRootScrollViewMarginType)type
{
    if ([self.rootScrollViewDelegate respondsToSelector:@selector(rootScrollView: marginForType:)]) {
        ...
        return SYPRootScrollViewDefaultMargin;
    }
}
...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < numberOfCells; i++) {
        ...
        SYPRootScrollViewCell *cell = self.displayingPageViews[@(i)];
